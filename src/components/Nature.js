...
const Nature =(props) => {
    return (
        <section className="Nature" id="nature">
            <h2>NATURE</h2>
            <div className="naturemain">
                <div className="natureBack">
                </div>
                <div className="natureActivity natureBeach">
                    <NatureActivity name='Beaches' show={[1,1,1,0]} images={[fiji,tahiti,borabora,maldivies]} places={['Fiji','Tahiti','Bora Bora','Maldivies']} onclick={props.searchTerm} searchDescription={['Fiji','Tahiti','Bora Bora','Maldivies']} searchCity = {['Suva, Fiji','Papeete','Vaitape','Malé,Maldives']} />
                </div>
                <div className="natureActivity natureNPark" >
                    <NatureActivity name='National_Parks' show={[1,1,1,1]} images={[Glacier,Yosemite,Yellowstone,GrandCanyon]} places={['Glacier','Yosemite','Yellowstone','Grand Canyon']} onclick={props.searchTerm} searchDescription={['Glacier National Park','Yosemite','Yellowstone','Grand Canyon']} searchCity = {['Columbia Falls','Yosemite','Yellowstone','Grand Canyon']} />
                </div>
                <div className="natureActivity natureSki">
                    <NatureActivity name='Ski_Areas' show={[1,1,1,1]} images={[Courchevel,Gstaad,Niseko,Telluride]} places={['Courchevel','Gstaad','Niseko','Telluride']} onclick={props.searchTerm} searchDescription={['Courchevel','Gstaad','Niseko','Telluride']} searchCity = {['Courchevel','Gstaad','Niseko','Telluride']} />
                </div>
                <div className="natureActivity natureRainforest">
                    <NatureActivity name='Rainforests' show={[1,1,1,1]} images={[Amazon,Daintree,ElYunque,TheCongo]} places={['Amazon','Daintree','El Yunque','The Congo']} onclick={props.searchTerm} searchDescription={['Amazon Rainforest','Daintre','El Yunque','The Congo Rainforest']} searchCity = {['Manaus,Brazil','eCairns,Australia','Río Grande','']} />
                </div>
                <div className="natureActivity natureWaterfalls">
                    <NatureActivity name='Waterfalls' show={[1,1,0,1]} images={[Victoria,Iguazu,Niagara,BanGiocDetian]} places={['Victoria','Iguazu','Niagara','Ban Gioc-Detian']} onclick={props.searchTerm} searchDescription={['Victoria Falls','Foz do Iguaçu','Niagara Falls','Ban Gioc-Detian']} searchCity = {['fittingly,Zimbabwe','Iguazu','Niagara','Trùng Khánh']} />
                </div>
                <div className="natureActivity natureLake">
                    <NatureActivity name='Lakes' show={[0,1,1,1]} images={[CraterLake,DeadSea,LagunaColorada,LakeComo]} places={['Crater Lake','Dead Sea','Laguna Colorada','Lake Como']} onclick={props.searchTerm} searchDescription={['Klamath','Ein Gedi','Potosi,Bolivia','Como,Italy']} searchCity = {['Crater Lake','Dead Sea','Laguna Colorada','Lake Como']} />
                </div>
                <div className="natureActivity natureGrassland">
                    <NatureActivity name='Grasslands' show={[1,1,1,1]} images={[AustralianSavanna,HulunBuir,CanadianPrairies,GreatPlains]} places={['Australian Savanna','Hulun Buir','Canadian Prairies','Great Plains']} onclick={props.searchTerm} searchDescription={['Australian Savanna','Hulun Buir','Canadian Prairies','Great Plains']} searchCity = {['','Hulunbuir','','']} />
                </div>
